"""
Fix IDENTITY property on id columns for all EDA MSSQL tables.
...
Usage (run from local machine with access to SQL Server):
...
conn = pyodbc.connect(
    ...
    'DATABASE=eda;UID=SA;PWD=AAP_P0C_Password_2026!;TrustServerCertificate=yes;',
    autocommit=False
)
c = conn.cursor()

SKIP_TABLES = {'dab_resource_registry_serviceid', 'awx_notify_messages'}
...
c.execute('''
    ...
    AND t.name NOT IN ('dab_resource_registry_serviceid', 'awx_notify_messages')
    ORDER BY t.name
''')
