-- Setup SQL Server database and notification bus for AAP Gateway
-- Replaces PostgreSQL pg_notify with polling table + Service Broker signal
-- Run against the SQL Server instance (master database initially, then USE aap_gateway)
-- ANSTRAT-1887 Phase 3+4: Gateway MSSQL Migration

-- 1. Create the gateway database
IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = 'aap_gateway')
    CREATE DATABASE aap_gateway;
GO
...
--    (dispatcherd, cache_clear) all see every message.
...
-- 3. Enable Service Broker on the gateway database
ALTER DATABASE aap_gateway SET ENABLE_BROKER WITH ROLLBACK IMMEDIATE;
ALTER DATABASE aap_gateway SET TRUSTWORTHY ON;
